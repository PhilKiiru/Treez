...
<body class="bg-light">
    ...
    <div class="container">
        ...
        echo "<h3 class='mt-5 mb-3'>View Orders</h3>";
        ...
        if (mysqli_num_rows($orders) > 0) {
            ...
            while ($row = mysqli_fetch_assoc($orders)) {
                $badge = ($row['ORDER_STATUS'] == "COMPLETED") ? "success" :
                         (($row['ORDER_STATUS'] == "PENDING") ? "warning text-dark" : "danger");

                echo "<tr>
                        <td>" . htmlspecialchars($row['ORDER_ID']) . "</td>
                        <td>" . htmlspecialchars($row['ORDER_DATE']) . "</td>
                        <td><span class='badge bg-$badge'>" . htmlspecialchars($row['ORDER_STATUS']) . "</span></td>
